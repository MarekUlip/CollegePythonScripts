...
def get_derivative(state, time, l1, l2, m1, m2):
    ...
    d_theta_1 = (m2 * g * np.sin(theta_2) * np.cos(theta_1 - theta_2) - m2 * np.sin(theta_1 - theta_2) * (
                l1 * (dot_theta_1 ** 2) * np.cos(theta_1 - theta_2) + l2 * dot_theta_2 ** 2) - (m1 + m2) * g * np.sin(
        theta_1)) / l1 * (m1 + m2 * np.sin(theta_1 - theta_2) ** 2)

    d_theta_2 = ((m1 + m2) * (l1 * (dot_theta_1 ** 2) * np.sin(theta_1 - theta_2) - g * np.sin(theta_2) + g * np.sin(
        theta_1) * np.cos(theta_1 - theta_2)) + m2 * l2 * (dot_theta_2 ** 2) * np.sin(theta_1 - theta_2) * np.cos(
        theta_1 - theta_2)) / l2 * (m1 + m2 * np.sin(theta_1 - theta_2) ** 2)
