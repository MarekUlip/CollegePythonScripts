"""
Script containing rule generation methods. Used in creating frequent rule sets.
"""
import copy
...
def create_variations():
    """
        Basic number variations generator for three dimensions
    """
    rules = []
    for i in range(6):
...
def create_rules(rule_len, num_of_possibilities, exclusions=None):
    """
        General version of create_variations. Generates number variation for specified dimension and specified number of
        possibilities.
    """
...
def create_rules_apriori(start_from, field_for_rule, rule_len, create_till, created_rules, transactions, minsup):
    ...
    for i in range(start_from, create_till):
        ...
        temp_field.append(i)
        create_rules_apriori(i + 1, temp_field, rule_len, create_till, created_rules, transactions, minsup)


def apriori(transactions, minsup):
    k = 0
    rules_to_create = []
    create_rules_apriori(1, [], 1, 75, rules_to_create, transactions, minsup)
    F = [rules_to_create]
    while len(F[k]) > 0:
        rules_to_create = []
        for rule in F[k]:
            create_rules_apriori(rule[len(rule) - 1] + 1, rule, k + 2, 75, rules_to_create, transactions, minsup)
...
def create_conf_rules(F, transactions, minsup):
    rule_sum = 0
    min_conf = 1
    conf_rules = []
    for rules in F:
...
print(len(create_conf_rules(apriori(dataset, 0.950), dataset, 0.950)))
# create_variations3()
# for i in range(5):
#    create_rules(i+1,75)
