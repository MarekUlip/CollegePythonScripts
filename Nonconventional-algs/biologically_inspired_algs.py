...
class AlgorithmGUI:
    def __init__(self, gui):
        ...
        self.search_algorithm_menu.add_command(label="Differential Evolution",
                                               command=partial(self.change_algorithm, 5))
        ...
        self.test_functions = [sphere_function, ackley_function, rosenbrock_function, schwefel_function,
                               styblinski_tang_function]

        self.selected_algorithm = 0
        self.algorithms_gui = [self.show_blind_search_gui, self.show_hill_climging_gui,
                               self.show_simulated_annealing_gui, self.show_soma_gui, self.show_particle_swarm_gui,
                               self.show_dif_evo_gui, self.show_evolution_strategy]
    ...
    def show_soma_gui(self, clear_screen=True):
        ...
        wrapper = Frame(self.main_frame)
        wrapper.pack(fill=X)
        Checkbutton(wrapper, text="Color Populations", variable=self.alg_params["color_pops"],
    ...
    def test_algorithms(self):
        print("\nTest function\t {}\t {}".format(self.alg_params["test_algorithms"][0].get(),
                                                 self.alg_params["test_algorithms"][1].get()))
        wrapper = Frame(self.main_frame)
        wrapper.pack(fill=X)
        tree = ttk.Treeview(wrapper, columns=(
        'Tested function', self.alg_params["test_algorithms"][0].get(), self.alg_params["test_algorithms"][1].get(),
        "Winner"))
        ...
        for index, test_func in enumerate(self.test_functions):
            ...
            for i in range(int(self.alg_params["num_of_tests"].get())):
                start = generate_identity(int(self.alg_params["dimension"].get()),
                                          float(self.alg_params["minimum_var"].get()),
                                          float(self.alg_params["maximum_var"].get()))
                results[0].append(self.start_alg_for_test(test_func, self.get_alg_index_by_name(
                    self.alg_params["test_algorithms"][0].get()), start))
                results[1].append(self.start_alg_for_test(test_func, self.get_alg_index_by_name(
                    self.alg_params["test_algorithms"][1].get()), start))
            winner = self.alg_params["test_algorithms"][0].get() if test_func(
                self.count_average(results[0])) < test_func(self.count_average(results[1])) else \
            self.alg_params["test_algorithms"][1].get()
            tree.insert('', 'end', text=self.test_function_names[index],
                        values=(self.count_average(results[0]), self.count_average(results[1]), winner))
            print(len(results[0]))
            print("{}\t {}\t {}\t".format(self.test_function_names[index], self.count_average(results[0]),
                                          self.count_average(results[1])))
            print("{}\t {}\t {}\t".format(self.test_function_names[index], test_func(self.count_average(results[0])),
                                          test_func(self.count_average(results[1]))))
        tree.pack(fill=BOTH)
        ...
        for index, test_func in enumerate(self.test_functions):
            ...
            for i in range(int(self.alg_params["num_of_tests"].get())):
                ...
                for i in range(int(self.alg_params["np"].get())):
                    start.append(generate_identity(int(self.alg_params["dimension"].get()),
                                                   boundaries=[int(self.alg_params["minimum_var"].get()),
                                                               int(self.alg_params["maximum_var"].get())]))
            ...
            winner = "rnd/1/bin" if test_func(self.count_average(results[0])) < test_func(
                self.count_average(results[1])) else "rnd/curt-to-best/bin"
            tree.insert('', 'end', text=self.test_function_names[index],
                        values=(self.count_average(results[0]), self.count_average(results[1]), winner))
            print(len(results[0]))
            print("{}\t {}\t {}\t".format(self.test_function_names[index], self.count_average(results[0]),
...
def soma(boundaries, dim, pop_size, migrations, step_size=0.11, path_length=3.0, prt=0.5, test_function=sphere_function,
         draw=True, color_pops=False, population=None):
    fitness = []
...
def soma_enhanced(boundaries, dim, pop_size, migrations, step_size=0.11, path_length=3.0, prt=0.5,
                  test_function=sphere_function, draw=True, color_pops=False, population=None):
    fitness = []
    points_to_draw = [[] for x in range(migrations)]
    if population is None:
        population = []
...
def particle_swarm(boundaries, dim, count_of_particles, iterations, test_function, c1=0.2, c2=0.2):
    ...
    for i in range(iterations):
        for j in range(len(particles)):
            particle_speeds[j] = count_speed(dim, particles[j], particle_speeds[j], personal_best[j], global_best, c1,
                                             c2, count_w(j, i + 1))
...
def count_speed(dim, position, prev_speed, personal_best, global_best, c1, c2, w):
    ...
    for i in range(dim):
        next_speed.append(
            w * prev_speed[i] + c1 * random.random() * (personal_best[i] - position[i]) + c2 * random.random() * (
                        global_best[i] - position[i]))
    return next_speed
...
def draw_graph_animated(minimum, maximum, test_function, points, res, interval=100):
    ...
    data = points[0]
    graph, = ax.plot([i[0] for i in data], [j[1] for j in data], [test_function([i[0], i[1]]) for i in data], color='k',
...
def draw_graph(minimum, maximum, test_function, points, res):
    ...
    for p in points:
        ax.plot([p[0]], [p[1]], [test_function([p[0], p[1]])], markerfacecolor='k', markeredgecolor='k', marker='.',
                markersize=8, alpha=1)

    ax.plot([res[0]], [res[1]], [test_function([res[0], res[1]])], markerfacecolor='r', markeredgecolor='r', marker='.',
            markersize=10, alpha=1)
    ax.plot([start[0]], [start[1]], [test_function([start[0], start[1]])], markerfacecolor='b', markeredgecolor='b',
...
def test_soma(dimension, test_func, num_of_tests):
    ...
    for x in range(num_of_tests):
        res = soma_enhanced(boundaries, dimension, pop_size, 25, test_function=test_func, draw=False,
                            population=population.copy())
        results.append(res)
        results_enh.append(res)

    for x in range(num_of_tests):
        res = soma(boundaries, dimension, pop_size, 25, test_function=test_func, draw=False,
                   population=population.copy())
    ...
    print("**********************")
    [print("{} = {}".format(test_func(x), x)) for x in
     results_normal]  # print(sum([test_func(x) for x in results])/num_of_tests) #([print(test_func(x)) for x in results]
    average = [0 for x in range(dimension)]
