"""
Script containing agglomerative and K-means clustering methods.
"""
import copy
import csv
import math
...
def get_two_closest_clusters(matrix):
    ...
    max_val = min(closest)
    cluster_num = np.where(closest == max_val)[0][0]
    sec_cluster_num = np.where(matrix[cluster_num] == max_val)[0][0]
...
def linkage(matrix, linkage_params, clust_index_a, clust_index_b):
    ...
    for index, row in enumerate(matrix):
        if index == clust_index_a or index == clust_index_b:
            continue
        new_cluster_row.append(linkage_type([row[clust_index_a], row[clust_index_b]]))
