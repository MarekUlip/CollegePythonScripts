...
def editational_distance(seq_a, len_a, seq_b, len_b):
    ...
    return min([editational_distance(seq_a, len_a - 1, seq_b, len_b) + 1,
                editational_distance(seq_a, len_a, seq_b, len_b - 1) + 1,
                editational_distance(seq_a, len_a - 1, seq_b, len_b - 1) + cost])
    ...
    for i in range(1, len_a):
        for j in range(1, len_b):
            matrix[i, j] = min([matrix[i - 1, j] + 1, matrix[i, j - 1] + 1,
                                matrix[i - 1, j - 1] + seqences_matches(seq_a, i - 1, seq_b, j - 1)])
